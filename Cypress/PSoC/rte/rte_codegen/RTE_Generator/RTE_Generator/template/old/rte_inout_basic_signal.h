 ...
 **/
STATIC_INLINE RC_t $Name/$_scl_to_val( const $Type_raw/$ * const praw, $Type_app/$ * const pvalue)
{
	*pvalue = ($Type_app/$) (*praw);
	return RC_SUCCESS;
}
 ...
 **/
STATIC_INLINE RC_t $Name/$_scl_to_raw( const $Type_app/$ * const pvalue, $Type_raw/$ * const praw)
{
	*praw = ($Type_raw/$) (*pvalue);
	return RC_SUCCESS;
}
...
{                                                                                                                      
   ...
   if ( NULL == p_sig_cfg->scl_to_raw_func )                                                                           
   {
       ...
       ret  = $Name/$_scl_to_raw(value, &signal->raw_value);
   }                                                                                                                   
   else                                                                                                                
   {                                                                                                                   
                           ...
                           , const $Type_app/$ value  )
{
   ...
   //Increment the age
   //$Name/$_inc_age(signal); //Taken out to support time triggered increment

   //Convert to raw value
   if ( NULL == p_sig_cfg->scl_to_raw_func )
   {
      ret = $Name/$_scl_to_raw(&value, &raw_value);
   }
   else
   {
                               ...
                               , const $Type_raw/$         raw_value )
{
   ...
   {
      ret = $Name/$_scl_to_val(&raw_value, &value);
   }
   else
   {
