...
/*****************************************************************************/
static const FunctionPtr pFunSevenSegement[] = 
{
    Pin_Seg1_A_Write,
    Pin_Seg1_B_Write,
    Pin_Seg1_C_Write,
...
RC_t SEVEN_init(void)
{
    ...
    /* Select 0th Display */
    Pin_Select_Write(SEGMENT_OFF);
    SEVEN_Write(0);
    
    /* Select 1st Display */
    Pin_Select_Write(SEGMENT_ON);
    SEVEN_Write(0);
...
{
    uint8 NoToDisplay = 0, LoopCounter = 0;
    /* Select the Display */
    Pin_Select_Write(DisplayNo);
    
    NoToDisplay = LookUpForDisplay[Value & MASK_MSB_UINT8];
    
    for (LoopCounter = 0; LoopCounter < SEVEN_NO_OF_SEGMENTS; LoopCounter++)
    {
        pFunSevenSegement[LoopCounter]((NoToDisplay >> LoopCounter) & BIT_MASK);
    }
